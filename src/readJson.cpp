...
void getParams( PARAMS * params)
{
	...
	if (item = cJSON_GetObjectItem(object, "icoord"))
		params->icoord = item->valueint;

	if (item = cJSON_GetObjectItem(object, "imedium"))
		params->imedium = item->valueint;
    ...
    if (!params->useTerrain)	
	    params->Depth = params->DH*params->NZ/1000.0;
